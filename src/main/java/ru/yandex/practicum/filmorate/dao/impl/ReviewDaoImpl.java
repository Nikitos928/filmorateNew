...
public class ReviewDaoImpl implements ReviewDao {
    ...
    private static final String SQL_FIND_REVIEW_BY_FILM_ID_REF = "SELECT r.* ," +
            "SUM(CASE WHEN rr.rate = 1 THEN 1 ELSE 0 END) - " +
            "SUM(CASE WHEN rr.rate = -1 THEN 1 ELSE 0 END) AS useful " +
            "FROM review r " +
            "LEFT JOIN review_rate rr ON r.review_id = rr.review_id " +
    ...
    private static final String SQL_FIND_REVIEWS_REF = "SELECT r.*, " +
            "SUM(CASE WHEN rr.rate = 1 THEN 1 ELSE 0 END) - " +
            "SUM(CASE WHEN rr.rate = -1 THEN 1 ELSE 0 END) AS useful " +
            "FROM review r " +
            "LEFT JOIN review_rate rr ON r.review_id = rr.review_id " +
    ...
    public List<Review> findALLReviews(Integer limit) {
        ...
        return jdbcTemplate.query(SQL_FIND_REVIEWS_REF, this::makeReview, limit);
    }
    ...
    public List<Review> findReviewsByFilmId(Integer limit, Long filmId) {
        ...
        return jdbcTemplate.query(SQL_FIND_REVIEW_BY_FILM_ID_REF, this::makeReview, filmId, limit);
    }
