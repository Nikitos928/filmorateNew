...
public class UserService {
    ...
    public void addToFriends(Long id, Long friendId) {
       ...
       // LocalDate dateTime = LocalDate.ofEpochDay(event.getTimestamp());
    ...
    public List<Film> getRecommendationFilms(long id) {
        ...
        Optional<Long> idBig = recommendationDao.getIdBig(id);
        if (idBig.isPresent()) {
            List<Long> bigListIdFilm = recommendationDao.getLikedFilm(idBig);
            List<Long> listIdFilm = recommendationDao.getLikedFilm(Optional.of(id));
            List<Long> listWithoutFilm = new ArrayList<>();
            for (Long aLong : bigListIdFilm) {
                boolean check = false;
                for (Long aLong1 : listIdFilm) {
                    if (Objects.equals(aLong, aLong1)) {
                        check = true;
                        break;
                    }
                }
                if (!check) {
                    listWithoutFilm.add(aLong);
                }
            }
            for (Long aLong : listWithoutFilm) {
                resultFilm.add(filmStorage.findFilmById(aLong));
            }
        }
        return resultFilm;
    }
